...
Each provider ships a set of `preferred_models` — the ids that appear in the model picker and the default model chosen when that provider is selected. Explicit provider choices are authoritative: if you pass `--provider`, Ante sends the model id you supplied to that provider even when it is not listed in `preferred_models` (see [Resolution](#how-a-model-and-provider-are-chosen)).
...
| `preferred_models` | — | Models shown for this provider in the picker and used as provider defaults (see [Model fields](#model-fields)). An empty or omitted list means the provider has no picker/default model of its own. |
