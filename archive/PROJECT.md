Below is a **Master Plan** structured as a Product Requirements Document (PRD) with two main sections: a **Plan** (the "what" and "why") and a **Design** (the "how"). This plan covers the Pi-agent capabilities you listed, the Claude Code-style hook system, and several additional features I identified to add significant, distinct value to Ante.

> **A note on repository status**: The latest analysis confirms that your fork at `realkotob/ante-preview` is **5 commits behind** the upstream `AntigmaLabs/ante-preview` and contains **0 unique commits of its own**. This makes it a perfect starting point for your feature branch.
